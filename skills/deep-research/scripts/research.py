...
class DeepResearchClient:
    ...
    async def start_research(self, query: str, format_spec: Optional[str] = None,
                             previous_interaction_id: Optional[str] = None) -> str:
        prompt = self._build_prompt(query, format_spec)
        client = await self._get_client()
        payload: Dict[str, Any] = {"input": prompt, "agent": self.AGENT, "background": True}
        ...
        try:
            ...
            if response.status_code != 200:
                raise DeepResearchError(f"API error {response.status_code}: {response.text}")
            data = response.json()
    ...
    async def wait_for_completion(self, interaction_id: str, timeout: Optional[int] = None,
...
async def cmd_research(args):
    ...
    try:
        ...
        interaction_id = await client.start_research(args.query, args.format, previous_id)
        ...
        result = await client.wait_for_completion(interaction_id, progress_callback=print_progress)
        print()
    ...
    if args.json:
        ...
        for item in interactions:
            status_icon = {"completed": "[ok]", "failed": "[!!]", "started": "[..]",
                           "streaming": "[>>]", "timeout": "[to]"}.get(item.get("status", ""), "[??]")
...
def main():
    parser = argparse.ArgumentParser(description="Gemini Deep Research - Autonomous multi-step research agent")
    cmd_group = parser.add_mutually_exclusive_group(required=True)
    cmd_group.add_argument("--query", "-q", help="Research query to execute")
    cmd_group.add_argument("--status", "-s", dest="interaction_id", metavar="ID", help="Check status")
    cmd_group.add_argument("--list", "-l", action="store_true", help="List recent tasks")
    parser.add_argument("--format", "-f", metavar="SPEC", help="Output format specification")
    parser.add_argument("--continue", dest="continue_from", metavar="ID", help="Continue from previous")
    parser.add_argument("--no-wait", action="store_true", help="Start without waiting")
    parser.add_argument("--json", "-j", action="store_true", help="JSON output")
    parser.add_argument("--raw", "-r", action="store_true", help="Raw API response")
    parser.add_argument("--limit", type=int, default=10, help="Number of recent tasks")
    args = parser.parse_args()

    if args.query:
        exit_code = asyncio.run(cmd_research(args))
    elif args.interaction_id:
        exit_code = asyncio.run(cmd_research(args))  # status check
    elif args.list:
